...
from oprael.utils.constants import MAXINT
from oprael.acquisition_function import *
from oprael.utils.util_funcs import get_types
...
def build_optimizer(func_str='local_random', acq_func=None, config_space=None, rng=None):
    ...
    if func_str == 'local_random':
        from oprael.acq_maximizer.ei_optimization import InterleavedLocalAndRandomSearch
        optimizer = InterleavedLocalAndRandomSearch
    elif func_str == 'random_scipy':
        from oprael.acq_maximizer.ei_optimization import RandomScipyOptimizer
        optimizer = RandomScipyOptimizer
    elif func_str == 'scipy_global':
        from oprael.acq_maximizer.ei_optimization import ScipyGlobalOptimizer
        optimizer = ScipyGlobalOptimizer
    elif func_str == 'mesmo_optimizer':
        from oprael.acq_maximizer.ei_optimization import MESMO_Optimizer
        optimizer = MESMO_Optimizer
    elif func_str == 'usemo_optimizer':
        from oprael.acq_maximizer.ei_optimization import USeMO_Optimizer
        optimizer = USeMO_Optimizer
    elif func_str == 'cma_es':
        from oprael.acq_maximizer.ei_optimization import CMAESOptimizer
        optimizer = CMAESOptimizer
    elif func_str == 'batchmc':
        from oprael.acq_maximizer.ei_optimization import batchMCOptimizer
        optimizer = batchMCOptimizer
    elif func_str == 'staged_batch_scipy':
        from oprael.acq_maximizer.ei_optimization import StagedBatchScipyOptimizer
...
def build_surrogate(func_str='gp', config_space=None, rng=None, transfer_learning_history=None):
    ...
    if func_str == 'prf':
        try:
            from oprael.surrogate.base.rf_with_instances import RandomForestWithInstances
            return RandomForestWithInstances(types=types, bounds=bounds, seed=seed)
        except ModuleNotFoundError:
            from oprael.surrogate.base.rf_with_instances_sklearn import skRandomForestWithInstances
    ...
    if func_str == 'random_forest':
        ...
        from oprael.surrogate.tlbo.mfgpe import MFGPE
